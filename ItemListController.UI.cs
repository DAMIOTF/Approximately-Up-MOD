...
namespace ApproximatelyUpMod
{
    public partial class ItemListController
    {
        ...
        private sealed class FlatModPanel : PanelBase
        {
            ...
            private Toggle _infiniteToggle;
            ...
            protected override void ConstructPanelContent()
            {
                ...
                CreateInfiniteToggle(resourcesSection);
            ...
            public void SyncRuntimeState()
            {
                ...
                if (_infiniteToggle != null && _infiniteToggle.isOn != InfiniteAllMaterials)
                {
                    _infiniteToggle.SetIsOnWithoutNotify(InfiniteAllMaterials);
                }
            ...
            private void CreateInfiniteToggle(GameObject section)
            {
                Text toggleText;
                GameObject toggleObject = UIFactory.CreateToggle(section, "InfinitePartsToggle", out _infiniteToggle, out toggleText, new Color(0.18f, 0.19f, 0.22f, 1f), 24, 24);
                UIFactory.SetLayoutElement(toggleObject, minHeight: 28, flexibleWidth: 9999);
                toggleText.text = "Infinite parts";
                toggleText.alignment = TextAnchor.MiddleLeft;

                _infiniteToggle.SetIsOnWithoutNotify(InfiniteAllMaterials);
                ((UnityEvent<bool>)(object)_infiniteToggle.onValueChanged).AddListener((UnityAction<bool>)delegate (bool isOn)
                {
                    ItemListController owner = OwnerController;
                    if (owner == null)
                    {
                    ...
                    if (InfiniteAllMaterials == isOn)
                    {
                        return;
                    }

                    InfiniteAllMaterials = isOn;
                    owner.SyncMaterialsState(force: true);
                    ModLog.Info("Infinite materials: " + (InfiniteAllMaterials ? "ON" : "OFF"));
                });
            }
